...
export class CreaterequestComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private snackbar: MatSnackBar, private api: ApiService, private dialog: MatDialog) {
  }
  ...
  sendDataRequest(message: string) {
    if (this.requestForm.value.startZip == null || this.requestForm.value.startCity == null
      || this.requestForm.value.startCountry == null || this.requestForm.value.destZip == null
      || this.requestForm.value.destCity == null || this.requestForm.value.destCountry == null
  ...
  add(): void {
    ...
    const dialogRef = this.dialog.open(AdditemComponent, {
      width: "350px",
      data: { description: this.description, size: this.size, weight: this.weight },
    });

    dialogRef.afterClosed().subscribe(result => {
      if(result.description == null || result.size == null || result.weight == null){
        this.snackbar.open('Alle Felder für den Gegenstand ausfüllen!', '',{duration: 1000});
      } else {
        console.log("The dialog was closed");
        this.itemsInput.push({ description: result.description, size: result.size, weight: result.weight });
      }
    });
  }
